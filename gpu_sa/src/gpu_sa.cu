...
__global__ void get_flip_energy( 
    int num_vars,
    std::int8_t *state,
    double* h,
    int* degrees,
    int** neighbors,
    double** neighbour_couplings,
    double* delta_energy
) {
    ...
    if (var < num_vars) {
        ...
        delta_energy[var] =  -2 * state[var] * energy;
    }
...
void simulated_annealing_run(
    ...
    cudaMalloc(&delta_energy, num_vars * sizeof(double));
    ...
    dim3 numBlocks(num_vars / threadsPerBlock.x);

    get_flip_energy<<numBlocks, threadsPerBlock>>(num_vars, state, h, degrees,
...
int general_simulated_annealing(
    ...
    int h_degrees[num_vars];
    for (unsigned int var = 0; var < num_vars; var++) {
        h_degrees[var] = degrees[var];
    }
    
    int* h_neighbors[num_vars];

    for (unsigned int var = 0; var < num_vars; var++) {
        h_neighbors[var] = (int*)malloc(neighbors[var].size() * sizeof(int));
        for (int i = 0 ; i < neighbors[var].size(); i++) {
            h_neighbors[var][i] = neighbors[var][i];
        }
    }
    
    double* h_neighbour_couplings[num_vars];

    for (unsigned int var = 0; var < num_vars; var++) {
        h_neighbour_couplings[var] = (double*)malloc(neighbour_couplings[var].size() * sizeof(int));
        for (int i = 0 ; i < neighbour_couplings[var].size(); i++) {
            h_neighbour_couplings[var][i] = neighbour_couplings[var][i];
        }
    }
